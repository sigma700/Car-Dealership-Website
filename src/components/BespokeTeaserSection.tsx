...
function EditorialCopy({inView}: {inView: boolean}) {
  return (
    <div className="flex flex-col justify-center h-full max-w-[420px]">
      ...
      {/* Headline – slightly reduced font size */}
      <div className="overflow-hidden mb-1">
        <motion.h2
          className="font-bold text-white leading-[0.91] tracking-[-0.03em]"
          style={{fontSize: "clamp(1.8rem, 3.6vw, 3.6rem)"}} // ← reduced from clamp(2rem, 4vw, 4rem)
      ...
      <div className="overflow-hidden mb-9">
        <motion.h2
          className="font-light text-[#BCBEC0]/42 leading-[0.91] tracking-[-0.01em] italic"
      ...
      >
        {["Private Consultation", "Worldwide Delivery", "Lifetime Support"].map(
          (item) => (
            <span
              key={item}
              className="flex items-center gap-1.5 text-[10px] tracking-[0.12em] text-[#BCBEC0]/20 uppercase font-medium"
            >
...
// ── Right column: scroll-driven image sequence (unchanged) ───────
